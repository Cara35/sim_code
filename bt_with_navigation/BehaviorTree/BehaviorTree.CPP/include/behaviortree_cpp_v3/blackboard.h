...
private:
  ...
  std::unordered_map<std::string, std::shared_ptr<Entry>> storage_; //保存键值对

  std::weak_ptr<Blackboard> parent_bb_; //指向父blackboard的指针，会与本blackboard有重映射关系。使用weak_ptr可避免循环引用

  std::unordered_map<std::string, std::string> internal_to_external_; //保存映射的内外对应关系
